...
export default function StoreSurveyPage({ params }: Props) {
  ...
  const handleAnswerChange = (
    ...
    // 현재 질문 정보를 찾습니다.
    const question = surveyQuestionsData.find((q) => q.id === questionId);
    if (!question) return; // 질문이 없으면 아무것도 하지 않음
    ...
    if (question.type === "select" && typeof value === "string") {
      ...
      // rating(number) 또는 textarea(string)
      newAnswers[questionId] = value;
    }

    // 계산된 새 객체로 상태를 업데이트합니다.
  ...
  if (currentOverallStep === 3) {
    return (
      <div className="container flex flex-col items-center justify-center min-h-screen p-4 mx-auto text-center bg-gray-100">
        ...
        <button
          onClick={() => {
            /* Reset or navigate home */ setCurrentOverallStep(1);
            setAnswers({});
            ...
            >
              ...
              <div>
                <label className="block mb-1.5 text-sm font-medium text-gray-700">
                  <VenetianMask className="inline w-4 h-4 mr-1.5 text-gray-500" />{" "}
                  성별
                </label>
              ...
              <div>
                <label className="block mb-1.5 text-sm font-medium text-gray-700">
                  <Cake className="inline w-4 h-4 mr-1.5 text-gray-500" />{" "}
                  나이대
                </label>
                <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                  {[
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setAge(option === "선택 안함" ? "" : option)
                      }
                      className={`px-3 py-2.5 border rounded-md text-sm font-medium transition-colors text-center ${
                        age === option || (option === "선택 안함" && age === "")
                          ? "bg-blue-600 text-white border-blue-600"
                          : "bg-white text-gray-700 border-gray-300 hover:bg-gray-100"
                      }`}
                    >
      ...
      if (question.type === "select" || question.type === "textarea") {
        return (
          <div className="w-full max-w-xl p-6 mx-auto bg-white rounded-lg shadow-lg sm:p-8">
            ...
            <label
              htmlFor={question.id}
              className="block mb-6 text-xl font-medium text-gray-800 leading-tight"
            >
              {questionText}
              {question.required && (
            ...
            {question.type === "select" && (
              <div className="space-y-3">
                {question.options?.map((opt, index) => {
                  ...
                  if (question.isMultiSelect && opt === "선택해주세요")
                    return null; // Don't render '선택해주세요' for multi-select
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`flex items-center justify-center w-6 h-6 border rounded-full transition-all duration-150 ease-in-out 
                        ${
                          isSelected
            ...
            {question.type === "textarea" && (
              <textarea
                ...
                placeholder={
                  question.placeholder || "자유롭게 의견을 작성해주세요..."
                }
                maxLength={200}
              />
            )}
        ...
        return (
          <div className="w-full max-w-xl mx-auto">
            <div key={question.id} className="mb-8 text-center">
              <label
                htmlFor={question.id}
                className="block text-2xl font-semibold text-gray-800 md:text-3xl leading-tight"
              >
              ...
              {question.labels && (
                <div className="flex justify-between mt-2 text-sm text-gray-500">
                  <span>
                    {question.labels[question.maxRating === 5 ? 1 : 0]}
                  </span>
                  <span>
                    {
                      question.labels[
                        question.maxRating ||
                          (question.maxRating === 10 ? 10 : 5)
                      ]
                    }
                  </span>
                </div>
    ...
    return (
      <div className="flex flex-col min-h-screen p-4 bg-gray-100 sm:p-6 md:p-8">
        ...
        <div className="flex flex-col items-center justify-center flex-grow w-full">
          <div className="w-full max-w-xl">{renderQuestionContent()}</div>
        </div>
        ...
        <div className="w-full max-w-md mx-auto mb-6">
          ...
          <div className="w-full p-1 bg-gray-300 rounded-full">
            <div
              ...
              style={{
                width: `${
                  ((currentQuestionIndex + 1) / surveyQuestionsData.length) *
                  100
                }%`,
              }}
            ></div>
        ...
        <div className="w-full max-w-xl px-4 pb-4 mx-auto border-t border-gray-300 pt-6 sm:px-0">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              ) : currentQuestionIndex === surveyQuestionsData.length - 1 ? (
